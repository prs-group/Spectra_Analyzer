...
#Version 1.1 24.11.20
#Changelog:
#only one band also possible right now
#    _._     _, -'""`-._
#   (,-.`._,'(  MR   |\`-/
#       `-.-' \ )-`( , o o)
#             `-    \`_`"'-
...
how_often = 359 #number of time_vars, files, imports = last number + 1 so if BB_137.0270.csv is the last file, then 271
name_sample = "BB_141" #Samplenumber for "Samplenumber.xxxx.csv"
...
want_animation = True #want animation? Gives Animation of both bands
second_plot = False
...
base_line_left = 1245.6
base_line_right = 1251.3

base_line_left_2 = 99999
base_line_right_2 = 99999
...
label_data_points = "Experimental Data at 1249.4 cm$^{-1}$"
label_data_points_2 = "Experimental Data at 1688.5 cm$^{-1}$"
size_legend = "x-small"
#name for fit has to be changed in line 384 and 385

#axis label plot
ax1_label = "Peak-area at 1249.4 cm$^{-1}$ / a.u. "
ax2_label = "Peak-area at 1688.5 cm$^{-1}$ / a.u. "
x_axis_label = "Time / h"
...
for i in range(how_often):
    if i < 10:
        ...
        area_straight = scipy.integrate.simps(straight_line_y, straight_line_x, even='avg')
        if second_plot == True:
            area_straight_2 = scipy.integrate.simps(straight_line_y_2, straight_line_x_2, even='avg')
        ...
        area_peak = scipy.integrate.simps(y_values_peak, x_values_peak, even='avg')
        if second_plot == True:
            area_peak_2 = scipy.integrate.simps(y_values_peak_2, x_values_peak_2, even='avg')
...
if second_plot == True:
    ...
    if want_legend == True:#legend if only 2 plots needed
